...
impl GifEncoder {
    ...
    pub fn write_frame_no_alloc(
        ...
        Self::write_frame_header(buf, frame, delay, interlaced, dispose);
        Self::write_image_block_no_alloc(temp, buf, &frame.buffer);
    }

    pub fn write_image_block(buf: &mut Vec<u8>, data: &[u8]) {
        let mut lzw = Vec::with_capacity(data.len() / 4);
        lzw_encode(&mut lzw, data);
        Self::write_encoded_image_block(buf, &lzw);
    }

    pub fn write_image_block_no_alloc(temp: &mut Vec<u8>, buf: &mut Vec<u8>, data: &[u8]) {
        temp.clear();
        temp.reserve(data.len() / 4);
        lzw_encode(temp, data);
        Self::write_encoded_image_block(buf, &temp);
    }
...
impl Frame {
    pub fn from_rgba(w: u16, h: u16, data: &[u8], speed: i32) -> Self {
        ...
        for pix in data.rchunks_exact(4) {
            if pix[3] == 0 {
...
impl GlobalPalette {
    ...
    pub fn index_of(&self, pix: &[u8]) -> usize {
        self.nq.index_of(pix)
    }
}
...
pub fn delay_from_millis(millis: u16) -> u16 {
    const MIN_DELAY: u16 = 10; // in ms
    const MAX_DELAY: u16 = 65535; // in 100ths of a second
    (millis.max(MIN_DELAY) / 10).min(MAX_DELAY)
}
